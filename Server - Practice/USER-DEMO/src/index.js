...
app.get("/users", (req, res) => {
  console.log("Request /users just comes");
  //   res.json(users);
  res.json({ users: users, total: 100, skip: 0, limit: 30 });
});

//Get User with Id
app.get("/users/:userId", (req, res) => {
  //console.log(req); //userId nằm trong params. Muốn log thấy thì truyền vào Postman.
  const user = users.find((item) => item.id === parseInt(req.params.userId));
  if (!user)
    res.json({ message: `User with id '${req.params.userId}' not found` });
  res.json(user);
});
